package ltd.rust_lang.utils.tree
    ...
    private fun reBalance(tmp: Node<E>?) {
        ...
        when (Pair(parent.isLeftChild(), node.isLeftChild())) {
            Pair(first = true, second = true) ->
                rotate(grandpa, node.left, node, node.right, parent, parent.right, grandpa, grandpa.right)
            Pair(first = true, second = false) ->
                rotate(grandpa, parent.left, parent, node.right, node, node.right, grandpa, grandpa.right)
            Pair(first = false, second = true) ->
                rotate(grandpa, grandpa.left, grandpa, node.left, node, node.right, parent, parent.right)
            Pair(first = false, second = false) ->
                rotate(grandpa, grandpa.left, grandpa, parent.left, parent, node.left, node, node.right)
        }
    }
